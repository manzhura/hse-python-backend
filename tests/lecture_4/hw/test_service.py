...
def test_register_user(client, user_info):
    ...
    # тест не корректного запроса
    second_response = client.post("/user-register", json=registration_data)
...
def test_get_user_by_user(client, user_info, entity):
    ...
    # тест корректного получения user
    ...
    assert invalid_user_fetch_response.status_code == HTTPStatus.BAD_REQUEST
    
    empty_params_response = client.post("/user-get", params={}, auth=credentials)
    assert empty_params_response.status_code == HTTPStatus.BAD_REQUEST
    ...
    assert non_existent_id_response.status_code == HTTPStatus.BAD_REQUEST

    non_credentials_response = client.post("/user-get", params={"id": user_id,  "username": entity.info.username})
    assert non_credentials_response.json()["detail"] == "Not authenticated"
    
def test_promote(client, user_info, admin_info):
    registration_data = user_info.model_dump()
    registration_data["password"] = user_info.password.get_secret_value()
    registration_data["birthdate"] = user_info.birthdate.isoformat()
    credentials = (user_info.username, user_info.password.get_secret_value())   
    ...
    promote_response = client.post("/user-promote", params={"id": user_id}, auth=credentials)
    assert promote_response.status_code == HTTPStatus.FORBIDDEN

    # тестирование от админки
    credentials_admin = (admin_info.username, admin_info.password.get_secret_value())
    promote_response = client.post("/user-promote", params={"id": user_id}, auth=credentials_admin)
    fetch_response = client.post("/user-get", params={"id": user_id}, auth=credentials_admin)
    assert promote_response.status_code == HTTPStatus.OK
    assert fetch_response.json()["role"] == "admin"

    fetch_response = client.post("/user-get", params={"username": user_info.username}, auth=credentials_admin)
    assert fetch_response.status_code == HTTPStatus.OK

    fetch_response = client.post("/user-get", params={"username": "random_user"}, auth=credentials_admin)
    assert fetch_response.status_code == HTTPStatus.NOT_FOUND

    random_promote_response = client.post("/user-promote", params={"id": 999}, auth=credentials_admin)
    assert random_promote_response.status_code == HTTPStatus.BAD_REQUEST
